...
// ============================================================================
// Tool Schema Macro Tests
// ============================================================================

#[test]
fn test_tool_schema_empty() {
    use k8s_mcp::mcp::protocol::ToolInputSchema;

    let schema = k8s_mcp::tool_schema!();
...
#[test]
fn test_tool_schema_with_properties() {
    use k8s_mcp::mcp::protocol::PropertySchema;

    let schema = k8s_mcp::tool_schema!(
        "name" => PropertySchema::string(),
        "count" => PropertySchema::integer()
    );
...
#[test]
fn test_prop_string() {
    let prop = k8s_mcp::prop_string!();
    assert_eq!(prop.type_, Some("string".to_string()));

    let prop_with_desc = k8s_mcp::prop_string!("A description");
    assert_eq!(
        prop_with_desc.description,
        Some("A description".to_string())
    );
}

#[test]
fn test_prop_integer() {
    let prop = k8s_mcp::prop_integer!();
    assert_eq!(prop.type_, Some("integer".to_string()));

    let prop_with_desc = k8s_mcp::prop_integer!("Count value");
    assert_eq!(prop_with_desc.description, Some("Count value".to_string()));
}

#[test]
fn test_prop_boolean() {
    let prop = k8s_mcp::prop_boolean!();
    assert_eq!(prop.type_, Some("boolean".to_string()));

    let prop_with_desc = k8s_mcp::prop_boolean!("Enable flag");
    assert_eq!(prop_with_desc.description, Some("Enable flag".to_string()));
}
